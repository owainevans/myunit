...
from analytics import VentureUnit as nUnit
...
if __name__ == '__main__':
    ...
    def runner(params):
       ...
       print params
       return LDA(ripl, params).runFromConditional(mySweeps,name='rfcLDA', runs=2, verbose=False)
    ...
    def runner2(params):
       print "Running setting " + str(run_count) + " of 8"
       print params
       return nLDA(ripl, params).runFromConditional(mySweeps,name='rfc_nLDA', runs=2, verbose=False)
    nhistories = productMap(parameters, runner2)
    
    #plotAsymptotics(parameters, nhistories, 'sweep_time', fmt='png', aggregate=True)
    sparams={'topics' : 3, 'vocab' : 5, 'documents' : 3, 'words_per_document' : 15}
    myLDA = LDA( shortcuts.make_puma_church_prime_ripl(),parameters=sparams)
    mynLDA = nLDA( shortcuts.make_puma_church_prime_ripl(),parameters=sparams)
    hl=myLDA.runFromConditional(400,runs=2)
    hn=mynLDA.runFromConditional(400,runs=2)
