...
def mk_u(assumes,observes):
    class MyClass(VentureUnit):
        ...
        def makeObserves(self):
            ...
            # for exp,val in observes:
            #     if exp.strip().startswith('<<'):
            #         self.observe(exp,self.parameters[exp])
            #     else:
            #         self.assume(sym,exp)
...
assumes=[('mu','(normal 0 5)'),('sigma','(gamma 1 .1)')]
observes=[('(normal mu sigma)','60.'),('(normal mu sigma)','60.')]


gauss_class = mk_u(assumes,observes)
model = gauss_class(mk_p_ripl())

history = model.runFromConditional(100)
